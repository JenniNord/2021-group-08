...
## Set-up
### Docker Build & Run
1. In a terminal, change to the directory with recording files
...
3. In a new terminal, enable access to the GUI
   1. `xhost +`
4. In the same terminal, run the h264decoder
   1. `docker run --rm -ti --net=host --ipc=host -e DISPLAY=$DISPLAY -v /tmp:/tmp h264decoder:v0.0.4 --cid=253 --name=img`
5. In a new terminal, clone repository into a directory of your choosing using SSH
   1. `git clone git@git.chalmers.se:courses/dit638/students/2021-group-08.git`
6. In the same terminal, change directory to the repository
   1. `cd 2021-group-08`
7. In the same terminal, build the repository
   1. `docker build -f Dockerfile -t driveryourself:latest .`
8. In the same terminal, run DriverYourself
